...
//===============================
const loadmaze = (socket, clients, args) =>{
    try {
        if (socket.player != null && args.length === 2) {
            let isMember = isUseradmin(socket.role);
     let usage = 0;
...
//  generateMaze(maze)
          let size = args[1]
          if (isMember){
         if (size > 50) {socket.player.body.sendMessage('you want to lag the server, max is 50: next time you will be kicked!'), usage =+ 1} else
           if (usage >1) {socket.kick(''), socket.talk('K', 'i said you will be kicked')} else
        generateMaze(size)

             
            } else{socket.player.body.sendMessage('must be admin or higher to load mazes.')}
        }
    } catch (error){
        util.error('[loadmaze()]');
        util.error(error);
      ...
      if (msg.author.id == owner_id, owner_id2) {
        entities.forEach(function(element) {
          if(element.type == 'bot' || element.type == 'tank' || element.type == 'food' || element.type == 'miniboss' || element.type=='crasher') {
            element.destroy();
            sockets.broadcast('all entities are killed by a developer!')
          }
        })
  ...
  if (msg.content.startsWith(prefix + 'define ')) {
   ...
   /*  if (msg.content.startsWith(prefix + 'kick ')) {
    ...
    if (msg.content.startsWith(prefix + 'kickdead ')) {
     ...
     if (msg.content.startsWith(prefix + 'countplayers')) {
           let lookfor =(msg.content.startsWith(prefix + 'countplayers'));
       let clients = sockets.getClients();
           let count = clients.length;
             console.log('done!');
           bot.createMessage(msg.channel.id, 'total player count is: '+ count);
  };
     if (msg.content.startsWith(prefix + 'countall')) {
           let lookfor =(msg.content.startsWith(prefix + 'countall'));
